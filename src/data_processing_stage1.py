...
class DataFrameProcessor:
    ...
    def load_data(self):
        DSDB = "ds"  # Use lowercase to match the actual schema name
        tableName = "ocod_full_2024_03"
        fullyQualifiedTableName = f"{DSDB}.{tableName}"

        # Check if the schema exists
        schemas = self.spark.sql("SHOW SCHEMAS")

        # Collect the schema names and convert to lowercase for case-insensitive comparison
        schema_list = [row['namespace'].lower() for row in schemas.collect()]
        
        if DSDB not in schema_list:
            print(f"Schema {DSDB} not found in the database.")
            sys.exit(1)
    ...
    def build_vae_model(self, input_dim, latent_dim):
        ...
        decoder_mean = Dense(input_dim, activation='sigmoid')  # Sigmoid for output between 0 and 1
...
if __name__ == "__main__":
    ...
    house_df_ori = house_df

    print(f"\n working on house_df_cleaned")
    house_df_cleaned = processor.cleanup_data(house_df, categorical_cols) 

    house_df_imputed = processor.impute_data_vae(house_df_cleaned)

    result_df = processor.query_house_df(house_df_ori)
    result_df.show(100, truncate=False)

    # Writing cleaned and imputed data back to Hive table
    print(f"\n\n house_df_cleaned from main has {house_df_cleaned.count()} rows\n")  
    print(f"\n\n house_df_imputed from main has {house_df_imputed.count()} rows\n")  
    #house_df_cleaned.printSchema()
    print(f"\n\n house_df_imputed from main\n")
    cleaned_df = processor.cleanup_columns(house_df_imputed)  
    #cleaned_df.show(1, False) 
    #house_df_imputed.select("*").show(1)
    #house_df_imputed.describe().show()
    # house_df_imputed.select([F.count(F.when(F.col(col).isNull(), 1).otherwise(0)).alias(col) for col in house_df_imputed.columns]).show()
    
    processor.write_data(cleaned_df, "house_df_cleaned_imputed")
    print(f"\n Performing processor.perform_analysis")
    processor.perform_analysis(house_df_imputed)

    print("Data loading, cleaning, and imputation completed successfully.")
    print(f"\nInitialize the reconstructor and reconstruct the dataframe\n")
    # Initialize the reconstructor and reconstruct the dataframe
    reconstructor = DataFrameReconstructor(spark)
    reconstructed_df = reconstructor.reconstruct_dataframe(house_df_imputed)
    reconstructed_rows = reconstructed_df.count()
    print(f"\n Total rows in reconstructed_df is {reconstructed_rows}\n")

    # Convert PySpark DataFrame to Pandas DataFrame
    # Convert to Pandas DataFrame (with potential optimizations for large datasets)
    limit_size = reconstructed_rows
    house_df_imputed_pandas = house_df_imputed.limit(limit_size).toPandas()  # Sample for testing

    # Identify non-numeric columns
    non_numeric_columns = house_df_imputed_pandas.select_dtypes(exclude=[np.number]).columns

    # Handle non-numeric columns: you can drop them or encode them if they are categorical
    # Here, we choose to drop them for simplicity
    house_df_imputed_pandas = house_df_imputed_pandas.drop(columns=non_numeric_columns)

    # Ensure the data is numeric and of type float32   
    # Convert to NumPy array
    data = house_df_imputed_pandas.values.astype(np.float32)
    print(f"\n house_df_imputed_pandas.count is {house_df_imputed_pandas.count()}\n")
    # Count the total number of rows in the DataFrame
    row_count = len(house_df_imputed_pandas)
    print(f"\nNumber of rows in house_df_imputed_pandas: {row_count}\n")

    # Check for NaNs or Infs
    if np.isnan(data).any() or np.isinf(data).any():
        raise ValueError("Data contains NaNs or Infs. Please clean the data before proceeding.")

    # Convert Pandas DataFrame to Spark DataFrame
    built_spark_df = spark.createDataFrame(house_df_imputed_pandas)

    #built_spark_df.select(col("PricePaid")).show(10, False)
    print(f"\nbuild_spark_df count is {built_spark_df.count()}")

    # Calculate and display the minimum and maximum values of the PricePaid column
    
    min_value = built_spark_df.agg(F.min("PricePaid").alias("min_price")).collect()[0]["min_price"]
    max_value = built_spark_df.agg(F.max("PricePaid").alias("max_price")).collect()[0]["max_price"]
    locale.setlocale(locale.LC_ALL, 'en_GB.UTF-8')  # Set locale to British English
    min_value_formatted = "{:,.2f}".format(min_value)
    max_value_formatted = "{:,.2f}".format(max_value)
 
    print(f"Minimum PricePaid: {min_value_formatted}")
    print(f"Maximum PricePaid: {max_value_formatted}")
    
    print(f"\n\n Working on augmentation with vae_imputer.fit\n\n")

    # Automatically set input_dim based on the data's number of features
    input_dim = data.shape[1]  # Set input_dim to the number of columns/features in your dataset
    intermediate_dim = 64  # Adjust based on model design
    latent_dim = 32  # Size of the latent space, adjust as needed

    # Create an instance of VAEImputer with the specified dimensions
    vae_imputer = VAEImputer(input_dim=input_dim, intermediate_dim=intermediate_dim, latent_dim=latent_dim)

    # Fit the VAE on the imputed data
    print(f"\n\n Started vae_imputer.fit(data)\n\n")
    print(np.isnan(data).any())  # Should return False
    print(np.isinf(data).any())  # Should return False
  ...
  # Check for NaN values
    ...
    total_count = data.size  # data.size gives the total number of elements in the array
    ...
    # Basic statistics
    mean = np.nanmean(data, axis=0)  # mean of each feature, ignoring NaNs
    std = np.nanstd(data, axis=0)    # std deviation of each feature, ignoring NaNs
    min_val = np.nanmin(data, axis=0) # minimum value of each feature, ignoring NaNs
    max_val = np.nanmax(data, axis=0) # maximum value of each feature, ignoring NaNs
    ...
    print(f"\n Doing vae_imputer.fit(data)")
    data = data[~np.isnan(data).any(axis=1)]
    vae_imputer.fit(data)  # This fits the imputer on the data

    print(f"\n\n Completed vae_imputer.fit(data), exiting\n\n")
    sys.exit(0)

    print("\n\n Initialize VAEDataAugmentor with the imputed data provided\n\n")
    # Initialize VAEDataAugmentor with the imputed data provided
    vae_data_augmentor = VAEDataAugmentor(vae_imputer, house_df_imputed)

    # Generate new samples
    new_samples = vae_data_augmentor.generate_samples()
    print(new_samples.shape)

    # Check if new_samples is 1D and reshape it to 2D if needed
    if len(new_samples.shape) == 1:
        new_samples = new_samples.reshape(-1, 1)

    # If new_samples has only 1 column, duplicate it to match the expected number of columns
    if new_samples.shape[1] == 1:
        new_samples = np.tile(new_samples, (1, house_df_imputed_pandas.shape[1]))

    # Ensure the generated samples have the correct number of columns
    assert new_samples.shape[1] == house_df_imputed_pandas.shape[1], \
        f"Expected {house_df_imputed_pandas.shape[1]} columns but got {new_samples.shape[1]}"

    # Convert new_samples to a DataFrame
    new_samples_df = pd.DataFrame(new_samples, columns=house_df_imputed_pandas.columns)

    # Print the first few rows of the new samples DataFrame
    print(new_samples_df.head())

    # Convert the Pandas DataFrame to a Spark DataFrame
    new_samples_spark_df = spark.createDataFrame(new_samples_df)

    # Show the Spark DataFrame
    new_samples_spark_df.show(1, False)


    # Visualize the distribution of each feature
    for feature in new_samples_df.columns:
